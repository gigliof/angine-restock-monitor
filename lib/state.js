...
function saveStates(s) {
  ...
  // Mode 0o600 restricts access to owner only. The mode arg to writeFileSync
  // only applies on file creation; explicit chmod enforces 0o600 even when
  // the tmp file pre-existed (e.g. from a crashed run with permissive perms).
  const data = JSON.stringify(s, null, 2);
  fs.writeFileSync(STATE_TMP, data, { mode: 0o600 });
  fs.chmodSync(STATE_TMP, 0o600);
