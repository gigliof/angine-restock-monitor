...
function buildMessages(product, reason) {
  const html = [
  ...
  try {
    ...
    if (err.response) {
      ...
      try {
        ...
        body = String(err.response.data).slice(0, 200);
      }
...
let waClient = null;
let waReady = false;
// Tracks an in-flight initialization so concurrent callers share one Client
// instead of each spawning their own (and a Chromium child process).
...
async function initWhatsApp() {
  ...
  try {
    return await waInitPromise;
  } catch (err) {
    // Reset so a retry can attempt a fresh init rather than re-await a settled rejection.
    waInitPromise = null;
    throw err;
  }
}
...
async function resolveChat(raw) {
  ...
  if (config.whatsapp.chatId) {
    ...
    if (byId) {
      log("Using WA_CHAT_ID: " + config.whatsapp.chatId);
      return byId;
    }
  ...
  if (contact) {
    ...
    try {
      const c2 = await contact.getChat();
      if (c2) return c2;
    } catch (_) {}
  }
...
async function sendWhatsApp(message) {
  ...
  if (!chat) {
    throw new Error(
      "Could not find WhatsApp chat for " +
...
async function notify(product, reason) {
  const msgs = buildMessages(product, reason);
...
async function runDebugTelegram() {
  ...
  try {
    ...
    console.log(
      "Bot verified: @" + me.data.result.username + " (" + me.data.result.first_name + ")"
    );
  } catch (err) {
    if (err.config) err.config = null;
    if (err.request) err.request = null;
  ...
  if (!chatId || chatId === "your-chat-id") {
    ...
    try {
      ...
      if (!msgs || msgs.length === 0) {
        console.log(
          "No messages found. Send any message to your bot in Telegram, then run this again."
        );
      } else {
        ...
        msgs.forEach((u) => {
          ...
          if (chat && !seen.has(chat.id)) {
            seen.add(chat.id);
            // sanitizeLog strips ANSI / control chars so a hostile chat name
            // can't write color codes or move the cursor in the operator's terminal.
            console.log(
              "  " +
                sanitizeLog(chat.first_name || chat.title || "?").padEnd(25) +
                " -> TELEGRAM_CHAT_ID=" +
                chat.id
            );
          }
...
async function runDebugWA() {
  ...
  // sanitizeLog wrapping defends the operator's terminal against ANSI / control
  // chars in attacker-controlled contact / chat names.
  ...
  contacts
    ...
    .forEach((c) => {
      console.log(
        "  " +
          sanitizeLog(c.name || c.pushname || "(no name)").padEnd(30) +
          " -> " +
          sanitizeLog(c.number)
      );
    });
  ...
  chats.slice(0, 15).forEach((ch) => {
    console.log(
      "  " + sanitizeLog(ch.name || "?").padEnd(30) + " -> " + sanitizeLog(ch.id._serialized)
    );
  });
  ...
  if (chat) {
    console.log(
      "  Found: " + sanitizeLog(chat.name) + " (" + sanitizeLog(chat.id._serialized) + ")"
    );
