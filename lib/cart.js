...
// NOTE: CTA_SELECTOR targets the pre-2026 buy button (a[href="#!"]). The redesigned
// site renders the in-stock add-to-cart control via the inventory-form Stimulus
// controller, which can't be observed from the static page while products are out of
// stock. If this selector no longer matches at restock time, clickBuyButton returns
// false and the caller aborts cleanly — the restock notification is still sent with
// the product URL (only the checkout pre-fill is skipped). Revisit with a real
// in-stock sample to confirm/replace the selector.
async function clickBuyButton(page) {
  ...
  return page.evaluate((sel) => {
    const btn = Array.from(document.querySelectorAll(sel)).find(
      (el) => /\d/.test(el.textContent) && /[$\u20AC\u00A3]/.test(el.textContent)
    );
    if (btn) {
      btn.click();
      return true;
...
async function fillCheckoutForm(page, f) {
  ...
  await fillField(
    page,
    ['input[name="address1"]', 'input[name="address"]', "#address1"],
    f.address1
  );
...
async function clickContinueToPayment(page) {
  await page.evaluate(() => {
    const btn = Array.from(document.querySelectorAll("button, input[type=submit], a")).find((el) =>
      /continuer|continue|paiement|payment/i.test(el.textContent + el.value)
    );
    if (btn) btn.click();
  });
  await page.waitForNavigation({ waitUntil: "networkidle2", timeout: 15000 }).catch(() => {});
...
async function addToCartAndGetCheckoutUrl(productUrl, productName) {
  ...
  try {
    ...
    page.on("request", (req) => {
      if (req.resourceType() === "document") {
        ...
        if (!url.startsWith("about:")) {
          let allowed = false;
          try {
            allowed = new URL(url).hostname === config.allowedHostname;
          } catch (_) {}
          if (!allowed) {
      ...
      .waitForFunction(
        (sel) =>
          Array.from(document.querySelectorAll(sel)).some((el) =>
            /ajout\u00e9/i.test(el.textContent)
          ),
        { timeout: 10000 },
        CTA_SELECTOR
      )
      .catch(() => {
        log("  Warning: could not confirm cart addition - continuing.");
      });

    log("  Item added to cart.");

    await page.goto(config.allowedOrigin + "go/cart", {
    ...
    try {
      checkoutHostOk = new URL(checkoutUrl).hostname === config.allowedHostname;
    } catch (_) {}
    if (!checkoutHostOk) {
